...
- **Stopping the camera between files blanks the next receive.** After a success the pipeline used to `track.stop()`, kill the WASM workers, hide the preview, and reload. The second `getUserMedia` on a phone often returns a live-looking preview that never decodes (tiny cap/s, 0 dec/s); iOS also paints black frames when the `<video>` is `display: none`. Capture pauses so the looping sender does not immediately re-send the same file; "Receive another file" re-arms the still-running camera.
- **A camera whose capture period shares factors with the 2k carousel phase-locks.** A ~10 KB file (k=4, cycle 8) sampled at ~7 cap/s against a 60 fps sender is every 8th displayed frame — unique seqs pile up, the bar hits 99%, and repair-half residues never peel (degree k XOR with zero sources). Skipping whole camera ticks does nothing: that skip is a multiple of the cycle, and it burns down while the WASM worker is still busy. The fountain now rotates the systematic sweep each cycle and turns the repair half into degree-1 on odd cycles, so every residue class still finishes. The sender also walks one extra 60 Hz tick every 7 grid refreshes; the receiver holds 20–110 ms after the worker frees rather than dropping camera ticks.
...
- **`<dialog>` is not a modal on iOS 14.** `showModal()` is Safari 15.4; without it the `open` attribute just unhides an in-flow box. `openDialog` injects a sibling backdrop and `.dialog-fallback` (fixed, centered) so Help / share still cover the camera.
- **`aspect-ratio` is Safari 15.** The camera box would collapse to the video's intrinsic height on iOS 14; `syncPreviewAspect` pins width/height in px when `CSS.supports("aspect-ratio")` is false. Overlay `top/right/bottom/left: 0` is written next to `inset` (Safari 14.1). `100dvh`/`100svh` declarations are preceded by `100vh`.
- **iOS ignores `<a download>`.** A blob URL in the same tab often navigates away from the receiver. On iOS the Save link opens a new tab so Share → Save to Files works. Copy uses `execCommand("copy")` when `navigator.clipboard` throws.
- **iOS 14 has no Wake Lock API.** `requestScreenWakeLock` is called on the Start-camera tap *before* any `await` (WASM init would spend the user gesture). Native `navigator.wakeLock` is used when present; otherwise a near-silent oscillator holds the idle timer. A live `getUserMedia` preview does not.
...
- **Backdrop-click close must be geometric** (`shared/dialog.ts`): the gaps between a dialog's children are also `event.target === dialog`, so the target check alone closes on ordinary taps. Engines without `showModal` get a sibling `.dialog-fallback-backdrop` instead.
